...
command -q xclip && abbr -a cpp xclip -selection clipboard && abbr -a pcp xclip -o
...
test -f $HOME/.tmonly.fish && source $HOME/.tmonly.fish
test -f $HOME/.fzf.fish && source $HOME/.fzf.fish
test -f $HOME/.local/bin/mise && ~/.local/bin/mise activate fish | source
